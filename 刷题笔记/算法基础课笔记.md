...
更新操作：`r = mid - 1` 或 `l = mid`，计算`mid`时为 `mid = l + r + 1 >> 1` （防止出现死循环）
...
假设初始区间是`[l, r]`，每次二分缩小区间，如果左边界要更新为`l = mid`，就应该使用模版2，如果左边界更新为`l = mid + 1`就使用模版1。

细节：二分查找时，首先要确定边界值，保证每次二分缩小区间时，边界值始终保持在内。
